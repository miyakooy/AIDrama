...
foreach ($process in Get-Process -Name "tensorslab-studio", "TensorsLab" -ErrorAction SilentlyContinue) {
...
foreach ($process in Get-CimInstance Win32_Process -ErrorAction SilentlyContinue) {
  ...
  if (
    ...
    (($name -and $name -like "TensorsLab-*-setup.exe") -and (Test-IsPathUnderDirectory -Path $exePath -Directory $normalizedReleaseDir))
  ) {
